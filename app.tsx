 id: q.id,
  ...
  const startRecording = async (id: number) => {
    try {
      const stream = await navigator.import React, { useState, useRef, useEffect } from 'react';
...
const App: React.FC = () => {
  const [states, setStates] = useState<TranscriptionState[]>(
    QUESTIONS.map(q => ({mediaDevices.getUserMedia({ audio: true });
    ...
    <div className="min-h-screen p-4 md:p-8 flex flex-col items-center max-w-5xl mx-auto">
      ...
      <header className="w-full text-center mb-12">
        ...
        <p className="text-slate-400 max-w-2xl mx-auto">
          An automated CEFR-based English assessment tool for developers and language learners. 
          Record three distinct answers to find your proficiency level.
        </p>
      </header>
